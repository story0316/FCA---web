/**
 * book-order-admin.js — 도서 구매 신청 승인 관리 (관리자)
 */
...
const LEGACY_BOD_IDS = new Set(['BOD001','BOD002','BOD003','BOD004','BOD005','BOD006','BOD007']);
...
function _load() {
  ...
  try {
    ...
    const cleaned = d.filter(r => !LEGACY_BOD_IDS.has(r.id));
...
function _draw() {
  ...
  const approved = all.filter(r => r.status === 'approved' || r.status === 'ordered');
...
<div style="padding:0">
  ...
  <div style="display:flex;border-bottom:1px solid var(--border);background:var(--card-bg);overflow-x:auto">
    ...
    <button class="boa-tab" data-tab="${k}"
  ...
  _root.querySelectorAll('.boa-tab').forEach(b =>
...
function _renderItems(list, mode) {
  ...
  const statusMeta = {
    ...
    ordered:  { label: '발주 완료', bg: '#EDE9FE', color: '#7C3AED' },
  };
  ...
  <div style="background:var(--card-bg);border:1px solid var(--border);border-radius:12px;
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:11px;color:#64748B;margin-top:2px">${r.author} · ${(r.price||0).toLocaleString()}원</div>
        <div style="font-size:11px;color:#94A3B8;margin-top:1px">${r.empName} · ${r.dept} · ${r.requestedAt}</div>
      </div>
    ...
    <div style="display:flex;gap:8px;margin-top:10px">
      <button class="boa-approve" data-id="${r.id}"
      ...
      <button class="boa-reject" data-id="${r.id}"
    ...
    <div style="margin-top:10px">
      <button class="boa-order" data-id="${r.id}"
        style="width:100%;background:#EDE9FE;color:#5B21B6;border:none;border-radius:8px;
               padding:8px;font-size:12px;font-weight:600;cursor:pointer">📦 발주 완료 처리</button>
    </div>` : ''}
  </div>`;
  }).join('');
}
...
function _bindEvents() {
  ...
  _root.querySelectorAll('.boa-approve').forEach(btn =>
    btn.addEventListener('click', () => {
      ...
      showToast('도서 구매 신청이 승인되었습니다.', 'success')
      addNotification({ type: 'success', title: 'Book Order (관리자)', body: '도서 구매 신청이 승인되었습니다.' });
      _draw();
    }));

  _root.querySelectorAll('.boa-reject').forEach(btn =>
    btn.addEventListener('click', () => {
      const list = _load();
      const idx  = list.findIndex(r => r.id === btn.dataset.id);
  ...
  _root.querySelectorAll('.boa-order').forEach(btn =>
    btn.addEventListener('click', () => {
      const list = _load();
      const idx  = list.findIndex(r => r.id === btn.dataset.id);
      if (idx >= 0) { list[idx].status = 'ordered'; list[idx].orderedAt = new Date().toISOString().slice(0,10); _save(list); }
      showToast('발주 완료로 처리되었습니다.', 'success')
      addNotification({ type: 'success', title: 'Book Order (관리자)', body: '발주 완료로 처리되었습니다.' });
      _draw();
    }));
}
