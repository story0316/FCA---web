/**
 * work-from-abroad-admin.js — 해외 근무 신청 승인 관리 (관리자)
 */
import { showToast } from '../../components/toast.js';
import { addNotification } from '../../components/notification-hub.js';

const LS = 'hr_work_from_abroad';

const LEGACY_WFA_IDS = new Set(['WFA001','WFA002','WFA003','WFA004','WFA005','WFA006']);
...
function _load() {
  ...
  try {
    ...
    const cleaned = d.filter(r => !LEGACY_WFA_IDS.has(r.id));
...
function _renderItems(list, mode) {
  ...
  <div style="text-align:center;padding:48px 20px;color:#94A3B8">
    ...
    <div style="font-size:12px">해외 근무 신청 내역이 없어요</div>
  </div>`;
  ...
  <div style="background:var(--card-bg);border:1px solid var(--border);border-radius:12px;
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:700;color:var(--text)">🌏 ${r.country} · ${r.city}</div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:8px">
      <div style="font-size:11px;color:#64748B">📅 ${r.startDate} ~ ${r.endDate}</div>
      <div style="font-size:11px;color:#64748B;text-align:right">⏱ ${days}일</div>
      <div style="font-size:11px;color:#94A3B8;grid-column:1/-1">목적: ${r.purpose}</div>
      <div style="font-size:11px;color:#94A3B8">신청일: ${r.requestedAt}</div>
    </div>
    ${r.status === 'pending' ? `
    <div style="display:flex;gap:8px;margin-top:4px">
      <button class="wfa-approve" data-id="${r.id}"
      ...
      <button class="wfa-reject" data-id="${r.id}"
...
function _bindEvents() {
  ...
  _root.querySelectorAll('.wfa-tab').forEach(b =>
    b.addEventListener('click', () => { _tab = b.dataset.tab; _draw(); }));

  _root.querySelectorAll('.wfa-approve').forEach(btn =>
    btn.addEventListener('click', () => {
      ...
      showToast('해외 근무 신청이 승인되었습니다.', 'success')
      addNotification({ type: 'success', title: 'Work From Abroad (관리자)', body: '해외 근무 신청이 승인되었습니다.' });
      _draw();
    }));

  _root.querySelectorAll('.wfa-reject').forEach(btn =>
    btn.addEventListener('click', () => {
      const list = _load();
      const idx  = list.findIndex(r => r.id === btn.dataset.id);
