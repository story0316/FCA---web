/**
 * language-class-admin.js — 어학 수강 신청 승인 관리 (관리자)
 */
import { showToast } from '../../components/toast.js';
import { addNotification } from '../../components/notification-hub.js';

const LS = 'hr_language_classes';

const LEGACY_LC_IDS = new Set(['LC001','LC002','LC003','LC004','LC005','LC006']);

const LANG_META = {
  '영어':  { icon: '🇺🇸', color: '#3B82F6' },
  '일본어':{ icon: '🇯🇵', color: '#EF4444' },
  '중국어':{ icon: '🇨🇳', color: '#F59E0B' },
  '독일어':{ icon: '🇩🇪', color: '#10B981' },
  '프랑스어':{ icon: '🇫🇷', color: '#8B5CF6' },
  '스페인어':{ icon: '🇪🇸', color: '#EC4899' },
};
...
function _load() {
  ...
  try {
    ...
    const cleaned = d.filter(r => !LEGACY_LC_IDS.has(r.id));
...
function _draw() {
  ...
  const totalApprovedAmt = approved.reduce((sum, r) => sum + (r.monthlyFee || 0) * (r.months || 0), 0);
...
<div style="padding:0">
  <div style="display:flex;border-bottom:1px solid var(--border);background:var(--card-bg);overflow-x:auto">
    ${tabList.map(([k, l]) => `
    <button class="lca-tab" data-tab="${k}"
  ...
  <div style="padding:14px 16px">
    <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-bottom:8px">
      ${[
        ...
        { l: '승인 총액', v: totalApprovedAmt.toLocaleString() + '원', c: '#3B82F6' },
      ].map(k => `
...
function _renderItems(list) {
  ...
  <div style="text-align:center;padding:48px 20px;color:#94A3B8">
    ...
    <div style="font-size:12px">어학 수강 신청 내역이 없어요</div>
  </div>`;
  ...
  return list.map(r => {
    ...
    const lang = LANG_META[r.language] || { icon: '🌐', color: '#64748B' };
    const total = (r.monthlyFee || 0) * (r.months || 0);
    return `
  <div style="background:var(--card-bg);border:1px solid var(--border);border-radius:12px;
       padding:14px;margin-bottom:8px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:700;color:var(--text)">${lang.icon} ${r.language} · ${r.level}</div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:8px;font-size:11px;color:#64748B">
      <div style="grid-column:1/-1">기관: <span style="color:var(--text);font-weight:600">${r.institution}</span></div>
      <div>월 수강료: <span style="color:#3B82F6;font-weight:600">${(r.monthlyFee||0).toLocaleString()}원</span></div>
      <div>기간: <span style="color:var(--text)">${r.months}개월</span></div>
      <div>시작: <span style="color:var(--text)">${r.startDate}</span></div>
      <div>종료: <span style="color:var(--text)">${r.endDate}</span></div>
      <div style="grid-column:1/-1;color:#10B981;font-weight:600">지원 총액: ${total.toLocaleString()}원</div>
    </div>
    <div style="font-size:11px;color:#94A3B8;margin-bottom:8px">신청일: ${r.requestedAt}</div>
    ${r.status === 'pending' ? `
    <div style="display:flex;gap:8px">
      <button class="lca-approve" data-id="${r.id}"
      ...
      <button class="lca-reject" data-id="${r.id}"
...
function _bindEvents() {
  ...
  _root.querySelectorAll('.lca-tab').forEach(b =>
    b.addEventListener('click', () => { _tab = b.dataset.tab; _draw(); }));

  _root.querySelectorAll('.lca-approve').forEach(btn =>
    btn.addEventListener('click', () => {
      ...
      showToast('어학 수강 신청이 승인되었습니다.', 'success')
      addNotification({ type: 'success', title: 'Language Class (관리자)', body: '어학 수강 신청이 승인되었습니다.' });
      _draw();
    }));

  _root.querySelectorAll('.lca-reject').forEach(btn =>
    btn.addEventListener('click', () => {
      const list = _load();
      const idx  = list.findIndex(r => r.id === btn.dataset.id);
